...
namespace Human_management
{
    public partial class frm_demo_phanmemchamcong : Form
    {
        ...
        {
            sql = string.Format("INSERT INTO public.tbd_chitietchamcong(manhansu, giovao, giora, sogiolamviec, ngayhientai) " +
                "VALUES ( '{0}', '{1}', '{2}', '{3}', '{4}');", txt_manhansu.Text, dTP_giovao.Value.TimeOfDay.ToString(@"hh\:mm\:ss"), dTP_giora.Value.TimeOfDay.ToString(@"hh\:mm\:ss"), 
                (dTP_giora.Value.TimeOfDay - dTP_giovao.Value.TimeOfDay).ToString(@"hh\:mm\:ss"), "now()");
